...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        title='Total Successfull Launches for %s'%(entered_site))
        return fig
...
def get_scatter_chart(entered_site, payload_mass):
    ...
    mass_min = payload_mass[0]
    mass_max = payload_mass[1]
    filtered_df = spacex_df[(spacex_df['Payload Mass (kg)']>=mass_min)&(spacex_df['Payload Mass (kg)']<=mass_max)]
    if entered_site == 'ALL':
